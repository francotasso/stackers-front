...
class Content extends Component {
  ...
  mostrarData() {
    ...
    if (this.state.estado) {
      ...
      switch (this.state.operacion) {
        case "V":
          contenedor = (
            <div className="alert alert-info">{this.state.mensaje}</div>
          );
          break;
        case true:
          contenedor = (
  ...
  handleSearchClick(e) {
    ...
    //let url = "http://localhost:7896/recaudaciones/detallada";
    if (
      this.state.nombre_apellido === "" &&
      this.state.concepto === "" &&
      this.state.recibo === "" &&
      this.state.dates2 === "" &&
      this.state.dates === "" &&
      ...
      fetch(url, {
        ...
        .then(responseJson => {
          this.setState({
            ...
            estado: true,
            operacion:
              responseJson.data !== null && responseJson.data.length !== 0,
            mensaje:
              responseJson.data !== null && responseJson.data.length !== 0
                ? ""
